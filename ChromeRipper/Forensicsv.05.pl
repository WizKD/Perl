...
#SELECT 
#urls.url, urls.title, urls.visit_count, urls.typed_count, urls.last_visit_time, urls.hidden, visits.visit_time, visits.from_visit, visits.transition
#FROM urls, visits
#WHERE
#urls.id = visits.url
...
visits.visit_time, visits.from_visit, visits.transition FROM urls, visits WHERE urls.id=visits.url") or die "Couldn't prepare: $!";
$sth->execute() or die "Counldn't execute: $!";
while(@rows = $sth->fetchrow_array()){
printf ("url: %s / title: %s / visit: %d / typed: %d / visit_time: %d / was the url hidden? %d\n\n", $rows[0], $rows[1], $rows[2], $rows[3], $rows[4], $rows[5]);
printf ("visit_time: %d / from_visit: %d / tranisition: %d /\n\n", $rows[6], $rows[7], $rows[8]);
