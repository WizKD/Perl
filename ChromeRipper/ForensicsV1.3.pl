...
use JSON -support_by_pp;
	...
	&openChromeCookies ($ChromeDB);	
		...
		printf ("\n\nGrabbing URL-bar searched terms\n");
		print'-'x20; print "\n";
	&openSearchTerms($ChromeDB); # Search terms
		print '-'x20; print "\n";
...
sub openChromeHistory{
	...
	if (-d $dbPath){ # if file actually has data? -s
		$dbHandle = DBI->connect("dbi:SQLite:$dbPath/History","","") or die ("\n\nSomething went wrong during the scrape [History-RECENT] " . DBI->errstr);

		my $sth = $dbHandle->prepare("SELECT urls.url, urls.title, urls.visit_count, urls.typed_count, urls.last_visit_time, urls.hidden, visits.visit_time, visits.from_visit, visits.transition FROM urls, visits WHERE urls.id=visits.url GROUP BY urls.last_visit_time") or die ("\n\nCouldn't prepare the SQL statement for history: $!\n\n");
...
sub openChromeDeepHistory{
	...
	if (-d $dbPath){
		...
		@rows=();
		$sth = ();
		$sth = $dbHandle->prepare("SELECT term, lower_term FROM keyword_search_terms") or die ("\n\nCouldn't prepare the SQL statement for search terms [DEEP]: $!\n\n");
		$sth->execute() or die ("\n\nCouldn't execute query: $!\n\n");
		
		while(@rows = $sth->fetchrow_array()){
			my $term = $rows[0]; my $lower = $rows[1];
			printf ("Archived term searched: $term - Lower term: $lower\n\n");
			$ensureGrabSearch++;
		}
		if ($ensureGrabSearch == 0){ printf ("No search terms [DEEP] were found in the database.\n"); }
		else{ printf ("Found $ensureGrabSearch search term [DEEP] artifact(s).\n"); }
...
sub openChromeLogins{
	...
	if (-d $dbPath){
		...
		$sth->execute() or die ("\n\nCouldn't execute query: $!\n\n");
		printf ("Note that logins with creation value of '0' may have been imported from a different browser.\nPasswords will NOT be shown.\n\n");
		while(@rows = $sth->fetchrow_array()){
			my $originURL=$rows[0]; my $usernameVal=$rows[1]; my $dateCreated=$rows[2];
			if ($dateCreated){$dateCreated=&convertUnix($rows[2]);}
			printf ("Username: %s - Created: %s - URL: %s\n", $usernameVal, $dateCreated, $originURL);
			$ensureGrab++;
		}
...
sub openSearchTerms{
	...
	if (-d $dbPath){
		$dbHandle = DBI->connect("dbi:SQLite:$dbPath/History","","") or die ("\n\nSomething went wrong during the scrape [Search terms-RECENT] " . DBI->errstr);

		my $sth = $dbHandle->prepare("SELECT term, lower_term FROM keyword_search_terms") or die ("\n\nCouldn't prepare the SQL statement for search terms [RECENT]: $!\n\n");
		$sth->execute() or die ("\n\nCouldn't execute query: $!\n\n");
		
		while(@rows = $sth->fetchrow_array()){
			my $term = $rows[0]; my $lower = $rows[1];
			printf ("Term searched: $term - Lower term: $lower\n\n");
			$ensureGrab++;
		}
		if ($ensureGrab == 0){ printf ("No search terms [RECENT] were found in the database.\n"); }
		else{ printf ("Found $ensureGrab search term artifact(s).\n"); }
		$sth->finish; # Tell DB that we're done
		$dbHandle->disconnect; # Be nice to the DB
	}
	else{ printf ("\n\nThe database for search terms does not exist in the default path.\nAre you sure the system has chrome on it?\n\n"); exit(); }
}
...
# ---- Aux. modules 
sub Greet{
	printf ("\nWelcome to ChromeRipper (version 1.3)\n");
...
sub help{
	...
	printf ("\t\tThe only extension allowed at this time: '.txt'.\n");
	...
	printf ("%10s : Display all archived history for Chrome.\n", '-deep');
