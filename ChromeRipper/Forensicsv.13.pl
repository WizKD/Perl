...
#add switches for -deep, -terminal, -f, move wall time grab, ensure help section is updated with new switches
...
use Data::Dumper;
	...
	my ($OSChecker, $ClientOS, $Argcounter, $isScriptAdmin, $ClientLogin, $isUserAdmin, $ChromeDB, $DefaultBrowser, $localgroups, $walltime );
	my ($printTo, $grabDeep, $fileOutName);
	my (@ChromeRecentHistoryRecords, @ChromeRecentLoginRecords);
	$OSChecker=$ClientOS=$Argcounter=$isScriptAdmin=$ClientLogin=$isUserAdmin=$ChromeDB=$DefaultBrowser=$walltime=0; #Set initial variables to zero, just in case
	$printTo=$grabDeep=$fileOutName=();
	die if (!($^O =~ /MSWin32/i)); # If we're not on Windows, die
	foreach (@ARGV){
		if ($_ eq '-f'){  
				while ($fileOutName != /(\w+.txt)/){
					printf ("Please provide a file name for your output (.txt): ");
					chomp ($fileOutName = <STDIN>);
				}
				$printTo=2;}
		if ($_ eq '-deep'){ $grabDeep=1;}
		if ($_ eq '-terminal'){ $printTo=1; }
		if ($_ eq '-h'){ &help; }
		$Argcounter++;
	}
	if (!$printTo){ &help();}
	...
	if ($grabDeep){&openChromeDeepHistory($ChromeDB); }
...
sub help{
	...
	printf ("%15s : Run the program, but display all results to the terminal. This requires interaction by the user.\n", '-terminal');
	printf ("%15s : Run the program, save to a file. The user must specify the file directly following this switch.\n", '-f [filename]');
